...
<html lang="en">
    <head>
        ...
        <title>Edit Service History - {{ config('app.name', 'Airport App') }}</title>
    ...
    <body class="font-sans antialiased">
        ...
        <div class="container-main">
            <div class="card-header-custom">
                <div>
                    <h2>
                        <i class="bi bi-pencil-square"></i> Edit Service History 
                        <span class="badge-edit">ID: #{{ $history->id }}</span>
                    </h2>
                </div>
            ...
            <div class="card-body-custom">
                ...
                <form action="{{ route('servicehistory.update', $history->id) }}" method="POST">
                    @csrf
                            ...
                            <div class="col-md-6 mb-4">
                                ...
                                <select class="form-select @error('passenger_id') is-invalid @enderror" id="passenger_id" name="passenger_id" required>
                                    ...
                                    @foreach($passengers as $passenger)
                                        <option value="{{ $passenger->id }}" {{ $history->passenger_id == $passenger->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6 mb-4">
                                ...
                                <select class="form-select @error('staff_id') is-invalid @enderror" id="staff_id" name="staff_id" required>
                                    ...
                                    @foreach($staff as $member)
                                        <option value="{{ $member->id }}" {{ $history->staff_id == $member->id ? 'selected' : '' }}>
                                            {{ $member->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 mb-4">
                                ...
                                <select class="form-select @error('ground_service_id') is-invalid @enderror" id="ground_service_id" name="ground_service_id" required>
                                    ...
                                    @foreach($groundServices as $service)
                                        <option value="{{ $service->id }}" {{ $history->ground_service_id == $service->id ? 'selected' : '' }}>
                                            {{ $service->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 mb-4">
                                ...
                                <select class="form-select @error('service_id') is-invalid @enderror" id="service_id" name="service_id" required>
                                    ...
                                    @foreach($services as $service)
                                        <option value="{{ $service->id }}" {{ $history->service_id == $service->id ? 'selected' : '' }}>
                                            {{ $service->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-4">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" step="0.01" min="0" class="form-control @error('cost') is-invalid @enderror" id="cost" name="cost" value="{{ $history->cost ?? '0.00' }}" required>
                                </div>
                                @error('cost')
                            ...
                            <div class="col-md-4 mb-4">
                                ...
                                <input type="date" class="form-control @error('service_date') is-invalid @enderror" id="service_date" name="service_date" value="{{ $history->service_date ? $history->service_date->format('Y-m-d') : date('Y-m-d') }}" required>
                                @error('service_date')
                                    <span class="invalid-feedback">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-4 mb-4">
                                ...
                                <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                                    ...
                                    <option value="in-progress" {{ $history->status == 'in-progress' ? 'selected' : '' }}>
                                        <i class="bi bi-hourglass-split"></i> In Progress
                                    </option>
                                    <option value="completed" {{ $history->status == 'completed' ? 'selected' : '' }}>
                                        <i class="bi bi-check-circle"></i> Completed
                                    </option>
                                    <option value="cancelled" {{ $history->status == 'cancelled' ? 'selected' : '' }}>
                    ...
                    <div class="form-section">
                        ...
                        <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes" rows="4" placeholder="Add any additional notes or comments...">{{ $history->notes }}</textarea>
                        @error('notes')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="d-flex gap-3 justify-content-end mt-5">
                        ...
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-check-lg"></i> Update Service History
                        </button>
                    </div>
                </form>
            </div>
